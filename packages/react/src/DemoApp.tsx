    ...
    >
      ...
      >
        ...
        <Panel title="格式切换 (getContent)">
          {(
            ["text/html", "text/markdown", "text/plain", "text/lake", "json"] as YuqueDocScheme[]
          ).map((scheme) => (
            <CmdBtn
              key={scheme}
              label={`get: ${scheme}`}
        ...
        <Panel title="上传配置 (console)">
          ...
          <p style={{ fontSize: 11, color: "#999", margin: 0 }}>提示：粘贴一张图片即可触发</p>
        </Panel>
      </div>
      ...
      >
        ...
        <span>
          内容长度: <strong>{content.length}</strong>
        </span>
        <span>
          isEmpty: <strong>{editorRef.current?.isEmpty() ? "true" : "false"}</strong>
        </span>
